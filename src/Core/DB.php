...
{
    /**
     * Váriavel estática que armazenará a conexão ao banco de dados num objeto PDO
     *
     * @var \PDO
     */

     /**
      * Retorna uma instância de conexão ao banco de dados, reusa se já houver uma estabelecida
      *
      * @return \PDO
      */
    private static $db;
    ...
    /**
     * Método estático que retorna o resultado de uma consulta SQL preparada ou não.
     * Retorna um vetor de dados (PDO::FETCH_ASSOC)
     *
     * @param string $sql Consulta preparada com ou sem parâmetros
     * @param array $params Prâmetros opcionais
     * @return array
     */
    ...
    public static function query(string $sql, array $params=[]) : \PDOStatement
    {
        try {
            ...
            if (!$st->execute($params)) {
                error_log("Erro ao executar o comando SQL:\n{$sql}\nParâmetros:\n" . var_export($params, true));
                throw new Exception('Falha ao executar o comando SQL');
            }
            return $st;
        } catch (\PDOException $e) {
            $msgErrorLog = sprintf('ERRO PDO: %s, na LINHA: %s, comando: \n%s\nParâmetros:\n%s',
                $e->getMessage(),
                $e->getLine(),
                $sql,
                var_export($params, true));
